...
{  ;// Class Begin
	...
	__New()
	{
		this.bind:={id:0, mode:0}, this.bits:={oldzh:0, oldzw:0, hBM:0}, this.Lib:={}, this.Cursor:=0
	}
	...
	FindText(&OutputX:="", &OutputY:=""  , x1:="", y1:="", x2:="", y2:="", err1:="", err0:=""  , text:="", ScreenShot:="", FindAll:=""  , JoinText:="", offsetX:="", offsetY:="", dir:=""  , zoomW:=1, zoomH:=1)
	{
	  if RegExMatch(OutputX, "i)^\s*wait[10]?\s*$")
	  {
		...
		Loop
		{
		  ...
		  While (ok:=this.FindText(&OutputX, &OutputY , x1, y1, x2, y2, err1, err0, text, ScreenShot, FindAll , JoinText, offsetX, offsetY, dir, zoomW, zoomH))
			and (found)
		  {
			v:=ok[1], x:=v[1], y:=v[2], w:=v[3], h:=v[4]
			Sleep(10)
			if this.FindText(0, 0, x, y, x+w-1, y+h-1        , err1, err0, text, ScreenShot, FindAll        , JoinText, offsetX, offsetY, dir, zoomW, zoomH)
			  return (this.ok:=ok)
		  }
		  if (!found and !ok)
			return 1
	  ...
	  {
		dir:=offsetX, offsetY:=JoinText, offsetX:=FindAll    , JoinText:=ScreenShot, FindAll:=text, ScreenShot:=err0    , text:=err1, err0:=y2, err1:=x2    , y2:=y1, x2:=x1, y1:=OutputY, x1:=OutputX
	  }
	  (err1="" && err1:=0), (err0="" && err0:=0)  , (ScreenShot="" && ScreenShot:=1)  , (FindAll="" && FindAll:=1)  , (JoinText="" && JoinText:=0)  , (offsetX="" && offsetX:=20)  , (offsetY="" && offsetY:=10)  , (dir="" && dir:=1)
	  ...
	  JoinText:=(mode=1 ? [s] : JoinText), s1:=Buffer(k*4), s0:=Buffer(k*4)  , ss:=Buffer(2*(w+2)*(h+2)), FindAll:=(dir=9 ? 1 : FindAll)  , allpos_max:=(FindAll or JoinText ? 10240 : 1)  , ini:={sx:x, sy:y, sw:w, sh:h, zx:zx, zy:zy, zw:zw, zh:zh, mode:mode, bits:bits, ss:ss, s1:s1, s0:s0, allpos_max:allpos_max, zoomW:zoomW, zoomH:zoomH} ; V1toV2: if 'ss' is NOT a UTF-16 string, use 'ss := Buffer(2*(w+2)*(h+2))'
	  Loop 2
	  {
		...
		if (!JoinText)
		{
		  allpos := Buffer(allpos_max*8) ; V1toV2: if 'allpos' is NOT a UTF-16 string, use 'allpos := Buffer(allpos_max*8)'
		  For i,j in info
		  Loop this.PicFind(ini, j, dir, &allpos, ini.sx, ini.sy, ini.sw, ini.sh)
		  {
			x:=NumGet(allpos, 8*A_Index-8, "uint") + zx        , y:=NumGet(allpos, 8*A_Index-4, "uint") + zy        , w:=j[2], h:=j[3], comment:=j[11]        , arr.Push({1:x, 2:y, 3:w, 4:h, x:x+w//2, y:y+h//2, id:comment})
			if (!FindAll)
			  Break 3
		  }
		}
		...
		For k,v in JoinText
		{
		  v:=RegExReplace(v, "\s*\|[|\s]*", "|")      , v:=StrSplit(Trim(v,"|"), (InStr(v, "|")?"|":""), " `t")      , this.JoinText(ini, arr, info2, v, offsetX, offsetY, FindAll      , 1, v.Length, dir, 0, 0, ini.sx, ini.sy, ini.sw, ini.sh)
	...
	JoinText(ini, arr, info2, text, offsetX, offsetY, FindAll  , index:="", Len:="", dir:="", minY:="", maxY:=""  , sx:="", sy:="", sw:="", sh:="")
	{
	; REMOVED:   local
	  allpos := Buffer(ini.allpos_max*8) ; V1toV2: if 'allpos' is NOT a UTF-16 string, use 'allpos := Buffer(ini.allpos_max*8)'
	  ...
	  {
		x:=NumGet(allpos, 8*A_Index-8, "uint")    , y:=NumGet(allpos, 8*A_Index-4, "uint"), w:=j[2], h:=j[3]    , (index=1) && (ini.x:=x, minY:=y, maxY:=y+h)
		if (index<Len)
		{
		  if this.JoinText(ini, arr, info2, text, offsetX, offsetY, FindAll      , index+1, Len, 5, (y1:=Min(y,minY)), (y2:=Max(y+h,maxY)), x+w      , (y:=Max(y1-offsetY,ini.sy)), 0, Min(y2+offsetY,ini.sy+ini.sh)-y)
		...
		else
		{
		  ...
		  w:=x+w-ini.x, x:=ini.x+ini.zx      , h:=Max(y+h,maxY)-Min(y,minY), y:=Min(y,minY)+ini.zy      , arr.Push({1:x, 2:y, 3:w, 4:h, x:x+w//2, y:y+h//2, id:comment})
	...
	; REMOVED:   local
	  ...
	  {
		DllCall("DeleteObject", "Ptr", bits.hBM)    , bits.hBM:=this.CreateDIBSection(zw, zh, bpp:=32, &ppvBits ?? 0)    , bits.Scan0:=(!bits.hBM ? 0:ppvBits)    , bits.Stride:=((zw*bpp+31)//32)*4    , bits.oldzw:=zw, bits.oldzh:=zh
	  }
	...
	; REMOVED:   local
	  ...
	  if (mode=5)
	  {
		...
		else
		{
		  ...
		  Loop n + (i:=-2)*0
			v.=(r[i+=3]-x1)|(r[i+1]-y1)<<16|(Floor("0x"        . StrReplace(r[i+2], "0x"))&0xFFFFFF)<<32 . "/"
		  w:=x2-x1+1, h:=y2-y1+1
		}
		len1:=n, len0:=0
	  }
	  else
	  {
		...
		{
		  r:=StrSplit(color,"@"), color:=r[1], n:=r.Length>1?Round(r[2],2)+(!r[2]):0, n:=Floor(512*9*255*255*(1-n)*(1-n))
		  if (mode=3)
	...
	{
	; REMOVED:   local
	  (!IsObject(this.Lib) && this.Lib:=[]), Lib:=this.Lib  , (!Lib[index] && Lib[index]:=[]), Lib:=Lib[index]
	  if (add_to_Lib)
	  {
		re := "<([^>\n]*)>[^$\n]+\$[^`"\r\n]+" ;"
		Loop Parse, comments, "|"
	...
	; REMOVED:   local
	  ...
	  While (min_X!="" and min_X<=max_X)
	  {
		...
		min_X:=LeftX+LeftW-(overlapW>LeftW//2 ? LeftW//2:overlapW)    , dx:=LeftX+LeftW+offsetX, ocr_Y:=LeftY    , (LeftY<min_Y && min_Y:=LeftY)    , (LeftY+LeftH>max_Y && max_Y:=LeftY+LeftH)
	  }
	...
	RangeTip(x:="", y:="", w:="", h:="", color:="Red", d:=2)
	{
	  ...
	  Loop 4
	  {
		i:=A_Index    , x1:=(i=2 ? x+w : x-d)    , y1:=(i=3 ? y+h : y-d)    , w1:=(i=1 or i=3 ? w+2*d : d)    , h1:=(i=2 or i=4 ? h+2*d : d)
	...
	; REMOVED:   local
	  ...
	  if FileExist(pic:=RegExReplace(text, "\*\S+\s+"))
		text:="|<>##10$" pic
	  if (ok:=this.FindText(&x, &y, x1+dx, y1+dy, x2+dx, y2+dy    , 0, 0, text, ScreenShot, FindAll))
	  {
	...
	; REMOVED:     local
		...
		s:="`nDllCall(`"SetWindowText`",`"Ptr`",A_ScriptHwnd,`"Str`",`"<AHK>`")`n"      . "`n`n" . StrReplace(s, "`r")
		Try
		{
	...
	*/
	...
	; REMOVED:     #NoEnv
	  ...
	  Case "Capture","CaptureS":
		...
		return
	  Case "State":
		return GetKeyState((arg1?"LButton":"RButton"),"P")      . "|" GetKeyState((arg1?"LButton":"RButton"))      . "|" GetKeyState("Ctrl","P")      . "|" GetKeyState("Ctrl")
	  Case "SelectPic":
		...
		Loop Files, A_Temp "\Ahk_ScreenShot\*.bmp"
		  Pics.Push(LoadPicture(v:=A_LoopFilePath))      , Names.Push(v), s.Push(RegExReplace(v, "i)^.*\\|\.bmp$"))
	  ...
	  Case "SelectBox":
		...
		{
		  this.GetBitmapWH(hBM, &w, &h)
		  ogchPic.Value := "*W" (w<800?0:800)        . " *H" (h<500?0:500) " HBITMAP:*" hBM
		}
		return
	  Case "OK2":
		SelectBox := ogcListBoxSelectBox.Value
	  ...
	  Case "GetRange":
		...
		{
		  s:=StrReplace(s, r, r[1] " " p "," r[3], , 0, 1)
		  ogcEditscr.Value := s
		}
		FindText_Main.Show()
		return
	  ...
	  Case "ColorDiff2Two":
		...
		rr:=(c>>16)&0xFF, gg:=(c>>8)&0xFF, bb:=c&0xFF    , n:=Format("{:06X}",(dR<<16)|(dG<<8)|dB)    , color:=StrReplace(c "-" n, "0x"), k:=i:=0
		Loop nW*nH
		{
		  c:=cors[++k], r:=(c>>16)&0xFF, g:=(c>>8)&0xFF      , b:=c&0xFF, ascii[k]:=v:=(Abs(r-rr)<=dR
	  ...
	  Case "OK","SplitAdd","AllAdd":
		...
		else
		{
		  ...
		  r:=StrSplit(Trim(StrReplace(result, ",", "/"),"/"),"/")      , x:=r[1], y:=r[2], s:="", i:=1
		  Loop r.Length//3
			s.="," (r[i++]-x) "/" (r[i++]-y) "/" r[i++]
		...
		. "`n    if (i<=2)"    . "`n      FindText().MouseTip(ok[i].x, ok[i].y)`n"
	  ...
	  Case "LButtonDown":
		...
		{
		  c:=((ascii[k]:=!ascii[k]) ? "Black":"White")      , Gui_("SetColor")
		}
		else
		{
